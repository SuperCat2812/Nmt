...
function generateNumberByPercent(config: PercentageConfig): Question {
  ...
  return {
    ...
    text: `${part} становить ${percent}% деякого числа. Знайдіть це число.`,
    options,
    correctAnswer: String(answer),
    solution: [
...
function generateIncreaseByPercent(config: PercentageConfig): Question {
  ...
  return {
    ...
    text: `Число ${original} збільшили на ${percent}%. Яке число отримали?`,
    options,
    correctAnswer: String(answer),
    solution: [
...
function generatePercentageChange(config: PercentageConfig): Question {
  ...
  return {
    ...
    text: `Значення змінилося з ${original} до ${newValue}. На скільки відсотків воно ${increase ? 'збільшилося' : 'зменшилося'}?`,
    options,
    correctAnswer: String(answer),
    solution: [
      { math: `|${newValue} - ${original}| = ${difference}` },
      {
        math: `\\frac{${difference}}{${original}} \\cdot 100\\% = ${answer}\\%`,
      },
    ],
  };
}
...
function generateSuccessiveChange(config: PercentageConfig): Question {
  ...
  return {
    ...
    text: `Число ${original} спочатку збільшили на ${percent1}%, а потім отримане число зменшили на ${percent2}%. Яке число отримали?`,
    options,
    correctAnswer: String(roundedAnswer),
    solution: [
      {
        math: `${original} \\cdot \\left(1 + \\frac{${percent1}}{100}\\right) = ${afterFirst}`,
      },
      {
        math: `${afterFirst} \\cdot \\left(1 - \\frac{${percent2}}{100}\\right) = ${roundedAnswer}`,
      },
    ],
  };
}
